...
public class MemberInsertServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(valid) {
			...
			case FAIL:
				message ="다시 시도해보세요";
				goPage = "/WEB-INF/views/member/memberForm.jsp"; // 디스패치로 이동해야한다. 
				break;
			default : // ok  --> 웰컴페이지  
				goPage="/";
		...
		if(redirect) {
			resp.sendRedirect(req.getContextPath()+goPage); // 응답데이터가 나갔다가 다시 들어옴 
		}else {
			req.getRequestDispatcher(goPage).forward(req, resp); // 서버안에서만 이동		
		}
