...
//	private static final long serialVersionUID = 1L;
	...
	@GetMapping("/")
	public 	String doGet(@RequestParam (required=false)String command, Model model, HttpServletResponse response) throws ServletException, IOException {
/*		1. 파라미터 꺼내서
		...
		//command의 값이 무엇인지 확인하기 --> 이넘의 파라미터
		 ...
		 if(StringUtils.isNotBlank(command)) {
			 try {
				...
				model.addAttribute("includePage", desc);
			} catch (Exception e) {
